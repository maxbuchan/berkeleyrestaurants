...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    return group_by_first([[find_closest(restaurant_location(loc), centroids), loc] for loc in restaurants])
    # END Question 4
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    return [mean([restaurant_location(loc)[0] for loc in cluster]), mean([restaurant_location(loc)[1] for loc in cluster])]
    # END Question 5
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(new_cluster) for new_cluster in new_cluster]
        # END Question 6
        n += 1
    return centroids
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        restaurant_named = restaurant_name(restaurant)
        if restaurant in reviewed:
            dictionary_values[restaurant_named] = user_rating(user, restaurant_named)
        else:
            dictionary_values[restaurant_named] = predictor(restaurant)
